...
#ifndef STASSID
#define STASSID "your wireless name SSID"
#define STAPSK  "your wirelss password"
#endif
...
IPAddress dns(192, 168, 1, 1);  //IP Address of your WiFi Router or DNS 
  ...
  if (client) {                             // If a new client connects,
    ...
    while (client.connected()) {            // loop while the client's connected
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n' || (header.equals(s_on))||(header.equals(s_off))) {                    // if the byte is a newline character
          // if the current line is blank, you got two newline characters in a row.
          // that's the end of the client HTTP request, so send a response:
          if (currentLine.length() == 0 || (header.equals(s_on))||(header.equals(s_off))) {
            ...
            if ((header.indexOf("GET /RELAY/on")  >= 0) || (header.equals(s_on))) {
              Serial.println("RELAY on");
              RELAYState = "on";
              digitalWrite(RELAY, LOW);
            } else if ((header.indexOf("GET /RELAY/off") >= 0) || (header.equals(s_off))) {
